...
Biến thể mở rộng chống che khuất & Gán trọng số theo khả năng quan sát, bổ sung token động, hợp nhất tại JPM và tăng cường che khuất tổng hợp & 89.3 & 95.1 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[!t]
\centering
\caption{Kết quả của các biến thể trong thiết lập fine-tune từ checkpoint ReID. Mỗi ô được viết dưới dạng mAP/Rank-1 (\%). Dấu ``--'' biểu thị mốc chưa có số liệu đầy đủ trong nguồn nháp.}
...
\textbf{Biến thể thay thế trực tiếp đặc trưng toàn cục} là phép thử cực đoan nhất, trong đó đặc trưng global bị thay thế hoàn toàn bởi đặc trưng patch đã được gán trọng số. Kết quả chỉ đạt $64.0\%$ mAP và $81.8\%$ Rank-1, thấp hơn rất xa so với mọi biến thể còn lại. Bằng chứng này cho thấy đặc trưng patch-level, dù đã được điều chỉnh theo khả năng quan sát, vẫn chưa đủ sức đóng vai trò đại diện chính cho bài toán ReID nếu tách khỏi nhánh global của TransReID. Do đó, chiến lược thay thế trực tiếp có thể được xem là không phù hợp đối với bài toán này.

\subsubsection{Biến thể hiệu chỉnh residual}

Sau khi loại bỏ hướng thay thế trực tiếp, nghiên cứu chuyển sang một chiến lược bảo thủ hơn là \textbf{biến thể hiệu chỉnh residual có cổng}. Ở biến thể này, nhánh patch chỉ đóng vai trò hiệu chỉnh biểu diễn global thông qua một cổng học được, nhờ đó tác động của nhánh mới được kiểm soát chặt hơn trong giai đoạn đầu huấn luyện. Bảng~\ref{tab:v7_detailed} trình bày toàn bộ quá trình hội tụ của biến thể này theo các mốc đánh giá có trong log.
...
\subsubsection{Biến thể mở rộng chống che khuất}

Bên cạnh hai cấu hình trên, một \textbf{biến thể mở rộng chống che khuất} cũng được khảo sát. Biến thể này kết hợp bốn thành phần đồng thời: gán trọng số theo khả năng quan sát, bổ sung token động (\textit{dynamic token enrichment}), hợp nhất tại JPM và tăng cường che khuất tổng hợp (\textit{synthetic occlusion augmentation}). Mục tiêu của cấu hình mở rộng không chỉ là tích hợp nhánh patch vào đặc trưng cuối, mà còn tăng cường tính bền vững của mô hình trước hiện tượng che khuất và tăng cường khả năng lan truyền thông tin từ các patch có độ tin cậy cao.

\begin{table}[!htbp]
\centering
\caption{So sánh baseline TransReID và biến thể mở rộng chống che khuất tại epoch 120 dưới thiết lập khởi tạo từ ImageNet.}
...
Biến thể mở rộng chống che khuất & \textbf{89.3} & 95.1 \\
\bottomrule
\end{tabular}
\end{table}

So với baseline cùng giao thức khởi tạo, biến thể mở rộng này tăng $0.4$ điểm mAP trong khi chỉ giảm rất nhẹ $0.1$ điểm Rank-1. Điều này cho thấy cấu hình mở rộng thực sự cải thiện chất lượng sắp hạng tổng thể, tức là cải thiện phần thứ tự truy hồi ngoài vị trí đầu tiên, dù không làm thay đổi đáng kể top-1 accuracy. Xét trên các số liệu hiện có, \textbf{đây là cấu hình đạt mAP cuối cao nhất trong nhóm huấn luyện với khởi tạo từ ImageNet}.

\subsection{Tổng hợp kết quả thực nghiệm}

Nhìn chung, chiến lược thay thế trực tiếp đặc trưng toàn cục bằng đặc trưng patch không phù hợp đối với bài toán ReID. Ngược lại, các cơ chế tích hợp có kiểm soát cho kết quả khả quan hơn: trong thiết lập fine-tune, biến thể hợp nhất toàn cục có trọng số đạt hiệu năng tuyệt đối cao nhất; trong thiết lập khởi tạo từ ImageNet, biến thể hiệu chỉnh residual có cổng cho thấy quá trình hội tụ ổn định, còn biến thể mở rộng chống che khuất đạt mAP cuối cao nhất.
