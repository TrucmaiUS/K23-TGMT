...
def resolve_pretrained_path(cfg, model_path):
    if model_path and os.path.exists(model_path):
        return model_path
    if cfg.TEST.WEIGHT and os.path.exists(cfg.TEST.WEIGHT):
...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory):
        ...
        if self.use_vis_weighting and self.vis_mode == 'global_fusion':
    ...
    def forward(self, x, label=None, cam_label= None, view_label=None):
        aux_outputs = None
        if self.use_vis_weighting and self.vis_mode in ('global_fusion', 'foreground_filter', 'residual_fusion'):
...
class build_transformer_local(nn.Module):
    ...
    def forward(self, x, label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        if self.use_vis_weighting and self.vis_mode == 'local_jpm':
