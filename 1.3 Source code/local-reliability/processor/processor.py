...
def _parse_model_outputs(model_outputs):
...
def _build_occluded_batch(img, cfg):
    occ_img = img.clone()
    batch_size, _, height, width = img.shape
    occ_mask = torch.zeros(batch_size, 1, height, width, device=img.device, dtype=img.dtype)
    ...
    for idx in range(batch_size):
        if torch.rand(1, device=img.device).item() > cfg.INPUT.REL_OCC_PROB:
            continue
        area = height * width
        for _ in range(10):
            target_area = torch.empty(1, device=img.device).uniform_(min_area, max_area).item() * area
            aspect_ratio = torch.empty(1, device=img.device).uniform_(min_aspect, 1.0 / min_aspect).item()
            occ_h = int(round(math.sqrt(target_area * aspect_ratio)))
            occ_w = int(round(math.sqrt(target_area / aspect_ratio)))
            if 0 < occ_h < height and 0 < occ_w < width:
                top = torch.randint(0, height - occ_h + 1, (1,), device=img.device).item()
                left = torch.randint(0, width - occ_w + 1, (1,), device=img.device).item()
                occ_img[idx, :, top:top + occ_h, left:left + occ_w] = fill_value
                occ_mask[idx, :, top:top + occ_h, left:left + occ_w] = 1.0
                break
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                if cfg.MODEL.RELIABILITY_PIPELINE:
                    occ_img, occ_mask = _build_occluded_batch(img, cfg)
                    clean_outputs = model(img, target, cam_label=target_cam, view_label=target_view)
                    occ_outputs = model(occ_img, target, cam_label=target_cam, view_label=target_view)
                    score, feat, clean_aux = _parse_model_outputs(clean_outputs)
                    occ_score, occ_feat, occ_aux = _parse_model_outputs(occ_outputs)
                    aux_outputs = {
                        'clean_aux': clean_aux,
                        'occ_aux': occ_aux,
                        'occ_score': occ_score,
                    ...
                    score, feat, aux_outputs = _parse_model_outputs(model_outputs)
